function findFirstAndLastPosition(arr, key) {
    ...
    if (ans[0] != -1) {
        ans[1] = search(arr, key, false); // logN time complexity --> logN + logN = 2logN = O(logN) beacuse we ignore constants in time complexity
    } // if first occurence is found then only search for last occurence otherwise no need to search
...
console.log(findFirstAndLastPosition([1, 2, 3], 2)); //[1,1]
